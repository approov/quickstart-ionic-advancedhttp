...
@implementation CordovaHttpPlugin {
    ...
    ApproovHttpPlugin *approovPlugin;
}
...
- (void)pluginInitialize {
    ...
    [ApproovHttpPlugin initializeHeaders];
    approovPlugin = [[ApproovHttpPlugin alloc] init];
}
...
- (void) initializeApproov:(CDVInvokedUrlCommand*) command {
    CDVPluginResult *pluginResult;
    [approovPlugin initializeApproov];
...
- (void) approovSetDataHashInToken:(CDVInvokedUrlCommand*) command {
    ...
    [approovPlugin setDataHashInToken:data];
...
- (void) approovSetBindingHeader:(CDVInvokedUrlCommand*) command {
    ...
    [approovPlugin setBindingHeader:header];
    CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}

- (void) getApproovLoggableToken:(CDVInvokedUrlCommand*) command {
    NSString *host = [command.arguments objectAtIndex:0];
    NSString *loggableToken = [approovPlugin getApproovLoggableToken: host];
    CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString: loggableToken];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
