...
abstract class CordovaHttpBase implements Runnable {
  ...
  public void run() {
    ...
    try {
      // potentially update the URL to account for any changes due to query parameter substitutions
      ApproovService approovService = this.tlsConfiguration.getApproovService();
      this.url = approovService.substituteQueryParams(this.url);
  ...
  protected void prepareRequest(HttpRequest request) throws JSONException, IOException {
    ...
    // update the request with the impact of Approov
    ApproovService approovService = this.tlsConfiguration.getApproovService();
