Cordova Advanced HTTP with Request Interceptors
===============================================

This plugin is based on the Cordova Advanced HTTP 2.5.1 plugin (see below) which it extends by a means to register
interceptor functions to be called immediately before the sending of each request. These interceptors can then perform
special handling and "last-minute" modification of requests. If the interceptor facility is not used, this plugin's
behaviour is unchanged with respect to the Cordova Advanced HTTP plugin.

This plugin has been updated to work with OkHttp 3.14.

The interceptor facility is made available by the native code implementation of the class CordovaHttpPlugin.
Interceptors can be added by using the following functions:

- Android/Java:
  In class CordovaHttpPlugin:
...
The readme of the original, unmodified Cordova Advanced HTTP is included below:
